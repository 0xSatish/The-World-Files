...
export default function TeamCard({ member, isHighlighted = false }: TeamCardProps) {
  ...
  return (
    <TiltCard className="h-full">
      <div
        id={`member-${member.id}`}
        className={`group relative rounded-2xl border transition-all duration-320 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1.5 hover:scale-[1.025] hover:z-20 h-full ${
          isOfficial
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="relative shrink-0">
            ...
            <div
              className={`relative overflow-hidden rounded-full border border-white/20 bg-bg-raised ${
                isOfficial ? "w-20 h-20 md:w-24 md:h-24" : isHouseFace ? "w-16 h-16 md:w-20 md:h-20" : "w-14 h-14"
              }`}
            >
              <Image
                src={member.image}
        ...
        <div className="mb-3">
          <div className="flex items-baseline justify-between gap-2">
            <h3
              className={`font-serif tracking-tight text-ink font-normal ${
                isOfficial ? "text-2xl md:text-3xl" : isHouseFace ? "text-xl md:text-2xl" : "text-lg md:text-xl"
              }`}
            >
              {member.name}
            </h3>
